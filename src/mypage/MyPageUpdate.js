...
function MyPageUpdate() {
    ...
    useEffect(
        () => {
            ...
            axios.get("/api/member/getSocialType").then(res => codeHandler(res)).then(res => {
                if (res != 'NONE') {
                    setSocial(true);
                }
            })
    ...
    function showCheckNick() {
        if (saveCheckNick?.nickName == "" || mynickCheck?.checkValue == "") {
            ...
            return <div style={{color: 'red', fontSize: '16px', fontFamily: 'SourceSansPro-Light'}}><b>중복되는 닉네임 입니다.</b>
            </div>
        } else {
            return <div style={{color: 'green', fontSize: '16px', fontFamily: 'SourceSansPro-Light'}}><b>사용 가능한 닉네임
                입니다.</b></div>
        }
    }
        ...
        <div>
            ...
            <form className="outer_my" style={{position: 'absolute'}}>
                ...
                <div className="outer_myContent">
                    ...
                    <MyDelete myprofile={myprofile} isSocial={isSocial}/>
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <button type="button" className="nickCheckBtn" class="btn btn-primary"
                            style={{borderRadius: '100px', width: '70px', marginLeft: '-30px'}} onClick={() => {
                        MyPageNickCheck();
                        submitCheckNick();
                    }}>체크
                    </button>
                    &nbsp;
                    <button type="button" className="nickCheckBtn" class="btn btn-primary"
                            style={{borderRadius: '100px', width: '70px'}} onClick={submitUpNickName}>변경
                    </button>
                    ...
                    <BsFillBrushFill style={{marginTop: "-58px", marginLeft: '180px'}}
                                     onClick={showIntro}></BsFillBrushFill>
                    {show ? <form>
                        <textarea name="updateCont" onChange={UpdateContentHandle} className="outer_Introdeuction"
                                  style={{height: '168.5px'}}>
                        </textarea> <BsCheckLg onClick={submitUpContent}
                                               style={{marginTop: '-410px', marginLeft: '210px'}}></BsCheckLg>
