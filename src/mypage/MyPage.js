...
function MyPage(props) {
    ...
    let userNo = props.menuParam; // 로그인한 사용자 userNo
    let initParam = null;
    ...
    let [myprofile, setMyProfile] = useState(initParam);
...
// 세션의 user_no와 파람의 user_no를 비교하여 해당하는 팔로워 보여주기.
    function followerComparison() {
        if (userNo === profile?.userNo) {
            return <MyFollowCount userNos={userNo}/>
        } else {
            return <AcceptFollowCount userNos={userNo}/>
        }
    }

    function followingComparison() {
        if (userNo === profile?.userNo) {
            return <MyFollowingCount userNos={userNo}/>
        } else {
            return <AcceptFollowingCount userNos={userNo}/>
        }
    }
                        ...
                        <div className="outer_MyProf">
                            <div className="outer_proimgf">
                                <img src={myprofile?.photo?.filePath + myprofile?.photo?.changeName}
                                     style={{marginLeft: '7px', marginTop: '0px'}} className="profileImg"></img>
                            </div>
                            <div className="outer_idf">
                                <p className="nickFontf" style={{marginTop: '7px'}}><b>{myprofile?.nickName}</b><img
                                    src={mark}
                                    style={{width: '25px', height: '25px', marginLeft: '7px', marginTop: '-5px'}}/></p>
                            </div>
                            <br/>
